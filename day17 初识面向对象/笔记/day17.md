...
- 面向对象，Python中支持两种编程方式来写代码，分别是：`函数式编程`、`面向对象式编程`。
  ...
  - 面向对象式编程
    ...
    ```python
    #p 先定义类(在类中定义方法实现功能)，再实例化类对象，再执行类中的方法
    # 定义类
    class Foo(object):
        # 在类中定义方法
        def func(self):
  ...
  Python支持两种编程方式（其他很多语言只支持一种），所以初学者在刚开始学习往往不知道应如何选择，并且行业内对于 函数式编程 vs 面向对象编程 之间谁更好的讨论也是难分胜负，其实在开发中无论要实现什么样的功能，两种编程模式都能实现，哪种让我们能更好实现就选择谁？不必非纠结于哪种方式更好，编程语言支持工具，最重要的是把功能实现。
...
# 2. 模块默认会将创建的那块区域的内存地址当作self参数传递进去，并执行__init__方法。    往区域中放入(data="注册成功")=给实例化对象创建变量data并赋值"注册成功"。
msg_object = Message("注册成功") #p 创建实例的时候就默认执行了__init__方法了，而且__init__方法是看不见的
...
- 实例变量，属于对象，**只能**通过对象调用。#p 即属于创建的实例的。
...
- 将数据封装到了对象中，在实例化一个对象时，可以通过`__init__`初始化方法在对象中封装一些数据，便于以后使用（即实例变量）。
...
class Foo(Base):
    def f2(self):
        print('before')
        self.f1() # 调用了f1方法   obj.f1()会优先去obj对应的类中找
        print('foo.f2')
...
class ThreadingTCPServer(ThreadingMixIn, TCPServer): # 左边的父类优先级最高，优先级从左往右降低
    def run(self):
        print('before')
        self.f1()
        print('after')
...
- self对应哪个对象？就去self对应的那个类中去获取成员（实例变量或绑定方法），没有就按照继承关系向上查找 。
...
在Python中则不一样，由于Python对数据类型没有任何限制，所以他天生支持多态（#p 比如传递给函数的参数可以是多种型态(字符串、列表、类等等)）。

```python
def func(arg):
    v1 = arg.copy() # 浅拷贝（字符串和列表都有该方法）
    print(v1)
    
func("武沛齐")  #p arg = "武沛齐"
func([11,22,33,44]) #p arg = [11,22,33,44]
```

```python
class Email(object):  #p 为啥括号里面有object？类定义时不是不要()嘛，只有继承时才有()。
    def send(self):
        print("发邮件")
...
def func(arg):
    v1 = arg.send() #p 管你arg是什么数据类型，只要传入的arg可以执行send方法就行
    print(v1)
    

v1 = Email() 
func(v1) #p 执行时去Email类里面找到send()方法执行      参数v1是个类

v2 = Message()
func(v2)#p 执行时去Message类里面找到send()方法执行
```

在程序设计中，鸭子类型（duck typing）是动态类型的一种风格。在鸭子类型中，关注点在于对象的行为，能作什么；而不是关注对象所属的类型，例如：一只鸟走起来像鸭子、游泳起来像鸭子、叫起来也像鸭子，那么这只鸟可以被称为鸭子。#p 不关注传入的参数本身，只要对其能操作就行；即不关注你本身是什么动物还是植物，只要你能嘎嘎叫(执行里面的方法)就认为是鸭子。在python里，这样的参数的数据类型就叫作鸭子类型。
...
# 实例化一个str类的对象v1
v1 = str("武沛齐")  
v1 = "武沛齐" #p 像字符串、列表等等比较常用，python内部已经提前设定好了，直接不用前面加类了。
