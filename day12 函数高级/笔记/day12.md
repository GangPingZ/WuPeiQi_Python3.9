...
- #p 把数据封装在==一个包==，当包里函数需要时就取。

  ```python
  #p 以前的写法，变量name直接设置为全局变量。
  name = "武沛齐"
  ...
  ```python
  #p 现在把变量封装在函数里面，因为有的函数其实是用不到这个变量的，如上面的f4函数。
  def func(age):
      name = "武沛齐"
...
- #p 把数据封装到==各个包==，当包里的函数需要时就取。
  
- ![image-20201229181719550](assets/image-20201229181719550.png)
  
  ```python
  def task(arg):           #p 第一步：创建函数
      def inner():
          print(arg)
      return inner   #p task函数返回的是内层函数名
  
  v1 = task(11)            #p 第二步：创建闭包
  v2 = task(22)
  v3 = task(33)
  v1()  #p 闭包内函数初始化   #p 第三步：使用闭包
  v2()
  v3()
  ```
...
装饰器，在不修改原函数内容的前提下，通过@函数**可以实现在函数前后自定义执行一些功能**（批量操作会更有意义）。
...
def auth(func):
    def inner(*args, **kwargs):
        # 判断用户是否已经登录，已登录则继续往下，未登录则自动跳转到登录页面。
...
def auth(func):
    #p inner.__name__ == func.__name__   inner.__doc__ == func.__doc__
...
handler()
print(handler.__name__)  # handler #p 伪造的更像了
```
