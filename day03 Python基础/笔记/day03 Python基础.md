...
所以，以后写代码时候，想要结束循环可以通过两种方式实现了，即：条件判断(条件语句里用break) 和 break关键字，两种在使用时无好坏之分，只要能实现功能就行。
...
name = "alex"
text = f"我是{ name.upper() }，我爱大铁锤" #z str.upper()字符串转换为大写
...
上述这3个优先级从高到低总结：`算数>比较>非与或 `。绝招：加括号(优先级最高)。
