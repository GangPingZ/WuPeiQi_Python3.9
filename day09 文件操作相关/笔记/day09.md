...
- 字符串类型（str），在程序中用于**表示文字信息**，本质上是==unicode编码中的二进制==。
...
file_object = open('info.txt', mode='rb') #z 该函数返回一个数据流对象，通过对该对象操作可以对该文件进行一些读写操作。
# 2°读取文件内容，并赋值给data
data = file_object.read() #z 以下所有的file_object指的是数据流对象(具体:文件对象)
# 3°关闭文件
file_object.close()
...
- 路径
  ...
  - 相对路径，你的程序到底在哪里运行的？
    ...
    <img src="assets/image-20201217140130973.png" alt="image-20201217140130973" style="zoom:50%;" />
...
readlines把整个⽂件中的内容进行一次性读取，并且返回的是一个字符串列表，其中每一行的数据为一个该字符串列表的一个元素(包括换行符)。
```
...
- f.readlines()，把整个文件中的内容进行一次性读取，并且返回的是一个字符串列表，其中每一行的数据为一个该字符串列表的一个元素(包括换行符)。
...
- f.writelines([str1, str2, ...])，向文件中写入多个指定字符串；若想换行自己加上换行符。
...
```python
os.remove(src) #z 可能会出现要删除的文件找不到的错误
```
...
```python
import os
os.mkdir(src) #z 当文件夹已存在时，再去创建同名的文件夹会出错
```
...
```python
os.getcwd() #z getcwd为 get current work directory,表示获取当前工作路径
```

#### 8.2.4 改变默认目录

切换当前路径到指定路径，用处不大。比如说你现在文件的路径为a，你需要跑到b路径去操作一些东西，需要用到这个函数
...
```python
os.listdir() #z 获取当前文件所处的文件夹下的所有文件，以列表形式返回
```
