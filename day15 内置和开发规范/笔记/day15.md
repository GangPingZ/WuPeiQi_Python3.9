...
- datetime
  ...
  v1 = datetime.now()  # 当前本地时间 #p 不是字符串类型 # utcnow()是世界时间
  print(v1)
  ...
  # datetime类型 - datetime类型    datetime类型之间只能减和比较
...
当给你一大堆**文本信息**，让你**提取其中的指定数据**时，可以使用正则来实现。例如：提取文本中的邮箱和手机号
