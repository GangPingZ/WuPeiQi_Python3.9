...
注意事项：以某个编码的形式进行保存文件，以后就要以这种编码去打开这个文件。否则就会出现==乱码==。
...
- 默认Python解释器是以UTF-8编码的形式打开文件。如果想要修改Python的默认解释器编码，可以这样干：在py文件第一行加上`# -*- coding:gbk -*-  `
...
一般我们提到的：5、18、22、99、… 等整数 在Python中都称为是整型。
...
- 乘，让整形和字符串进行相乘，以实现让字符串重复N次并拼接起来。
...
1 > "alex"  #z 不同数据类型之间是无法进行比较大小；但有些是支持相互转换的
```

### 4.4 int str bool类型之间转换
...
# 字符串转换为整形（超度可度之人）
  "6" + "9" 的结果是： "69"
  int("6") + int("9") 的结果是：15 ，但是int("alex是sb") 报错。
...
bool("alex") True
bool("砂玻帮你") True
bool("") False #z 这叫“空字符串”
bool(" ") True #z 这个不是空字符串
```

三句话搞定int str bool类型转换：

- 其他所有类型转换为布尔类型时，除了整数0、空字符串、空列表、空元组、空字典以外其他都是True。

- 字符串转整形时，只有那种 "988" 格式的字符串才可以转换为整形，其他都报错。

- 想要转换为哪种数据类型，就用这个数据类型的英文包裹一下就行。 
...
- 不能用Python内置的关键字
  ...
  python内置关键字：*[‘and’, ‘as’, ‘assert’, ‘break’, ‘class’, ‘continue’, ‘def’, ‘del’, ‘elif’, ‘else’, ‘except’, ‘exec’, ‘finally’, ‘for’, ‘from’, ‘global’, ‘if’, ‘import’, ‘in’, ‘is’, ‘lambda’, ‘not’, ‘or’, ‘pass’, ‘print’, ‘raise’, ‘return’, ‘try’, ‘while’, ‘with’, ‘yield’]*
...
```python
name = "wupeiqi"
new_name = name #z 变量名new_name会自动指向变量名name所指向的内存
```
...
```python
name = "wupeiqi"
new_name = name #z 这种赋值：直接建立新变量名与内存空间的指向关系
name = "alex"   #z 这种赋值：先创建内存空间存放字符串，再建立变量名与内存空间的指向关系
```
...
```python
num = 18
age = str(num) #z 先创建内存空间存放字符串，再建立变量名与内存空间的指向关系
```
